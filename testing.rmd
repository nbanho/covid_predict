...
# subset to test how to
BEL <- df %>%
  dplyr::filter(id == "BEL") %>%
  dplyr::filter(date >= "2020-09-01" & date <= "2021-01-01") %>%
  dplyr::select(date, new_confirmed)
BEL_train <- dplyr::filter(BEL, date <= "2020-10-20")
BEL_test <- BEL %>% dplyr::filter(date > "2020-10-20") %>% head(21)
```
...
BEL_pred <- cbind(date = BEL_test$date, data.frame(proj)) %>%
  reshape2::melt(c("date")) 

ggplot(mapping = aes(x = date)) +
  stat_lineribbon(data = BEL_pred, mapping = aes(y = value)) +
  geom_line(data = BEL_test, mapping = aes(y = new_confirmed)) +
  scale_fill_brewer()
```
...
```{r}
# train
cori.fit <- train.cori(BEL_train$new_confirmed, n1 = 100, n2 = 10, seed = 1)

# predict
cori.proj <- predict.cori(estimate_R_obj = cori.fit, d = 1e3, n = 21, seed = 1)
```
...
for (j in 1:length(countries)) {
  ...
  # R estimates
  trained_cori <- train(df_ctry$date, df_ctry$new_confirmed, method = "cori", n1 = 100, n2 = 10)
...
for (j in 1:length(countries)) {
  ...
  for (k in 1:nrow(test_df_ctry)) {
    ...
    trained_model <- train.arima(train_df_ctry$new_confirmed, iter = 500)
    trained_model2 <- train.arima(log(train_df_ctry$new_confirmed/train_df_ctry$population*1e5), iter = 500)
    preds <- predict.arima(trained_model, d = 1000)
    preds2 <- predict.arima(trained_model2, d = 1000)
    mean_preds <- rowMeans(preds)
    q95 <- apply(preds, 1, quantile, probs = .99)
    q95_2 <- apply(preds2, 1, function(x) quantile(exp(x) * train_df_ctry$population[1] / 1e5, probs = .99))
    if(any(q95 > 1e5)) {
      bad_model = trained_model
      stop("High pred")
      break
    }
    pred_mat[k, ] <- mean_preds
...
X <- df %>%
  dplyr::filter(id == countries[1]) %>%
  dplyr::select(new_confirmed) %>%
  slice(1:100) %>%
  as.matrix()
n <- nrow(X)
D <- distance(X)
...
Y <- rmvnorm(4e3, mean = c(X), sigma = Sigma)
plot(rowMeans(Y), type = "l")
```
