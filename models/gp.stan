functions {
  ...
  matrix gp_periodic_cov1(real[] x, real alpha, real rho, real period) {
    int N = num_elements(x);
    matrix[N, N] K;
    for (i in 1:N) {
      K[i,i] = alpha^2;
      for (j in (i + 1):N) {
        K[i,j] = (alpha ^ 2) * exp(-2*(sin(pi() * fabs(x[i] - x[j]) / period) ^ 2) / (rho ^ 2));
        K[j,i] = K[i,j];
      }
    }
    return K;
  ...
  matrix gp_periodic_cov2(real[] x1, real[] x2, real alpha, real rho, real period) {
    int N1 = num_elements(x1);
    int N2 = num_elements(x2);
    matrix[N1, N2] K;
    for (i in 1:N1) {
      for (j in 1:N2) {
        K[i,j] = (alpha ^ 2) * exp(-2*(sin(pi() * fabs(x1[i] - x2[j]) / period) ^ 2) / (rho ^ 2));
      }
    }
    return K;
  }
  ...
  vector gp_pred_rng(real[] x2,
                              ...
                              real alpha2, real rho2, real period,
                              //real alpha3, real rho3,
    ...
    vector[N2] f2;
    {
      ...
      matrix[N1, N1] k2_x1 = gp_periodic_cov1(x1, alpha2, rho2, period);
      ...
      matrix[N1, N2] k2_x1_x2 = gp_periodic_cov2(x1, x2, alpha2, rho2, period);
      ...
      matrix[N2, N2] k2_x2 = gp_periodic_cov1(x2, alpha2, rho2, period);
...
transformed parameters {
  matrix[N1, N1] L_K;
  {
    ...
    matrix[N1, N1] K2 = gp_periodic_cov1(x1, alpha2, rho2, period);
...
generated quantities {
  vector[N2] f2 = gp_pred_rng(x2, y1, x1, alpha0, rho0, alpha1, rho1, alpha2, rho2, period, sigma, delta);
  vector[N2] y2;
  for (n2 in 1:N2) {
