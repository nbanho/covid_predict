...
# models
models <- c("arima", "prophet", "cori", "gp")
...
df_hot <- do.call(rbind, map(files, function(f) {
  ...
  read_files(f) %>%
  get_max_N() %>%
  dplyr::filter(max_N >= 8) %>%
  rowwise() %>%
  mutate_at(vars(models), ~ hotspot_proba(., new_confirmed, q = 0.25)) %>%
  dplyr::select(id, n, models, max_N) %>%
  mutate_at(vars(n), ~ list(.[1:(max_N-7)])) %>%
  unnest(cols = c("n", models), names_sep = "_")
}))

df_hot_pl <- df_hot %>%
  rename(hotspot = cori_hotspot) %>%
  dplyr::select(n, contains("proba"), hotspot) %>%
  melt(c("n", "hotspot")) %>%
  mutate(variable = gsub("_proba", "", variable)) %>%
  group_by(n, variable) %>%
  summarize(AUC = auc(hotspot, value)[1]) %>%
  ungroup() %>%
  ...
  scale_color_brewer(palette = "Dark2") +
  scale_y_continuous(limits = c(.4, .7), breaks = seq(0, 1, .1), labels = function(x) x * 100) +
...
save_plot(df_hot_pl, "results/hotspot.pdf", w = 21, h = 14)
```
