...
```{r}
# read forecasts
df_ex_full <- read_forecasts(ex_files)
...
mean10 <- do.call(rbind, lapply(files, function(f) {
  ...
  # filter data
  df$N <- sapply(df$data, nrow)
  df <- dplyr::filter(df, N >= n_ahead)
  df$data <- lapply(df$data, function(D) D[n_ahead, ])
  df <- do.call(rbind, df$data)
  
  # compute mean forecast
  df$forecast <- sapply(df$forecast, mean)
  df$state_id <- tolower(s) 
  df$variable <- m
...
mean10_pl <- mean10 %>%
  ...
  facet_wrap(~ state_id, ncol = 3, scales = "free_x") +
