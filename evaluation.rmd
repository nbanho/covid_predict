...
model_names <- c("EpiEstim", "EpiNow2", "ARIMA", "Prophet", "GP")
...
```{r}
# compute CRPS
df_crps <- compute_forecast_score(files, type = "crps")
...
crps_by_phase_sum <- df_crps %>%
  left_join(phase_labs %>% dplyr::select(-start_date, -end_date) %>% rename(state_id = state)) %>%
  group_by(n, epidemic_phase, variable) %>%
  summarize(value = mean(value)) %>%
  ungroup()

# plot crps by phase
crps_by_phase_pl <- crps_by_phase_sum %>%
  rbind(df_crps_sum %>% mutate(epidemic_phase = "total")) %>%
  mutate(phase = firstup(epidemic_phase %>% as.character())) %>%
  mutate(phase = factor(epidemic_phase, levels = c("Total", "Exponential growth", "Subexponential growth",
