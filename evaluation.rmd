...
model_names <- c("Cori", "ARIMA", "Prophet", "GP")
...
save_plot(ny_pl_wtitle, "results/prob_forecast_ca.png", w = 16, h = 16) 
save_plot(ny_pl, "results/prob_forecast_ca.pdf", w = 16, h = 12) 
```
...
mCalib_pl <- df_mCalib %>%
  ...
  labs(x = "Incidence (new cases per 100,000 people)", 
       y = expression(CDF[Forecast]-CDF[Observed])) +
  theme_bw2() +
  theme(legend.title = element_blank()) 
...
df_hot_pl <- df_hot %>%
  ...
  scale_y_continuous(breaks = seq(0.4, 0.8, .1)) +
  labs(x = "Number of days forecasted ahead", 
       y = "AUC (%)") +
  theme_bw2() +
  theme(legend.title = element_blank()) 
...
```{r}
p1_perf <- crps_by_phase_pl + labs(title = "a")
p2_perf <- crps_by_state_pl + theme(legend.position = "none") + labs(title = "b")
p3_perf <- df_hot_pl + theme(legend.position = "none") + labs(title = "c")
perf_sum_pl <- grid.arrange(p1_perf, grid.arrange(p2_perf, p3_perf,ncol = 2),
