...
n_brks <- c(0, 7, 14, 21)
n_lbs <- c("1-week forecast", "2-weeks forecast")
...
```{r}
ca_1week_pl <- inci %>%
  dplyr::filter(state_id == "NY",
                n == 1) %>%
  ggplot(aes(x = date)) +
  stat_lineribbon(aes(y = forecast), point_interval = "mean_qi", .width = c(.95), size = 1) +
  geom_line(aes(y = incidence), linewidth = 1, color = "red") +
  facet_wrap(~model) +
  scale_fill_brewer() +
...
df_pCalib <- compute_forecast_score(files, type = "calibration", sum_by = c(0,7,14,21))

# plot score
pCalib_pl <- df_pCalib %>%
  mutate(n = paste0("Week: ", n)) %>%
...
```{r}
df_cov90 <- compute_forecast_score(files, type = "coverage", sum_by = c(0,7,14,21), q = 0.05) %>%
  group_by(n, variable) %>%
  summarize(value = sum(value) / n()) %>%
  ungroup()

df_cov50 <- compute_forecast_score(files, type = "coverage", sum_by = c(0,7,14,21), q = 0.25) %>%
  group_by(n, variable) %>%
  summarize(value = sum(value) / n()) %>%
  ungroup()
...
df_sharp <- compute_forecast_score(files, type = "sharpness", sum_by = c(0,7,14,21)) 

# summarize sharpness
df_sharp_sum <- df_sharp %>%
  group_by(n, variable) %>%
  summarize(value = mean(value, na.rm = T)) %>%
  ungroup()

# plot sharpness
sharp_pl <- df_sharp_sum %>%
  ggplot(aes(x = n, y = value, color = variable)) +
  geom_line() +
  geom_point(shape = 1) +
  scale_x_continuous(breaks = seq(1,3,1)) +
...
```{r}
# compute bias
df_bias <- compute_forecast_score(files, type = "bias", sum_by = c(0,7,14,21))
...
```{r}
p1_descr <- pCalib_pl + guides(color = guide_legend(nrow = 1, byrow = T)) + labs(title = "a")
p2_descr <- cov50_pl + theme(legend.position = "none") + labs(title = "b")
p3_descr <- cov90_pl + theme(legend.position = "none") + labs(title = "c")
p4_descr <- sharp_pl + theme(legend.position = "none") + labs(title = "d")
p5_descr <- bias_pl + theme(legend.position = "none") + labs(title = "e")
...
crps_pl <- df_crps %>%
  ...
  stat_dots(aes(color = variable), quantiles = 20) +
  stat_pointinterval(point_interval = mean_qi, size = 1 / cm(1), shape = 16, fill = "black", .width = NA) + 
  stat_pointinterval(point_interval = median_qi, size = 1 / cm(1), shape = 15, fill = "black", .width = NA) + 
  labs(x = "CRPS") +
  scale_color_manual(values = mcols) +
  scale_fill_manual(values = mcols) +
  facet_wrap(~ n) +
...
save_plot(crps_pl, "results/crps-distribution.pdf", w = 16, h = 8)
...
# plot
crps_hist_pl <- ggplot() +
  geom_histogram(data = df_crps, mapping = aes(x = value2+1), color = "gray", size = .33) +
  facet_grid(rows = vars(variable), cols = vars(n), switch = "y") +
  scale_x_log10(breaks = c(1, 1e1, 1e2, 1e3), 
                limits = c(1, NA),
                labels = c("0", "10", "100", ">1,000")) +
  scale_fill_brewer() +
  labs(x = "CRPS") +
  geom_vline(data = df_crps_sum, mapping = aes(xintercept = Median, y = 100), 
             color = "blue", linetype = "dashed", alpha = .5) +
  geom_point(data = df_crps_sum, mapping = aes(x = Median, y = 100), fill = "lightblue", shape = 22) +
  theme_bw2() +
...
df_crps_tot$n <- paste("Week", df_crps_tot$n)
...
save_plot(crps_by_phase_pl, "results/crps_by_phase.pdf", w = 14, h = 10)
```
...
df_crps_by_state_sum <- df_crps %>%
  group_by(state, state_id, variable) %>%
  summarize(Median = median(value2)) %>%
  ungroup() 
...
crps_by_state_pl <- df_crps_by_state_sum %>%
  ...
  geom_bar(stat="identity") +
  labs(y = "Median CRPS", x = "Number of days forecasted ahead") +
  theme_bw2() +
  theme(legend.title = element_blank()) +
  theme(axis.title.x = element_blank())
...
save_plot(crps_by_state_pl, "results/crps_by_state.pdf", w = 14, h = 10)  
```
...
```{r}
sens <- function(true, pred) {roc(true, pred, levels = c(0, 1), direction = "<")$sensitivities} 
inv_spec <- function(true, pred) {1-roc(true, pred, levels = c(0, 1), direction = "<")$specificities} 

# compute auc
df_hot_auc <- df_hot %>%
  filter(!is.na(hot)) %>% # remove change in incidence < 0.25
  mutate(week = paste0("Week ", ifelse(n <= 7, 1, ifelse(n <= 14, 2, 3)))) %>%
  group_by(n, variable) %>%
...
#   summarize(sensitivity = sens(hot, prob_hot),
#             inv_specificity = inv_spec(hot, prob_hot)) %>%
#   ungroup() %>%
#   left_join(df_hot_auc) %>%
#   mutate(variable = paste0(variable, " (AUC = ", round(100 * AUC), ")")) 
# 
# # plot roc
# df_roc %>%
#   ggplot(aes(x = inv_specificity, y = sensitivity, color = variable)) +
#   facet_wrap(~ week) +
#     geom_path() +
#     geom_abline(intercept = 0, slope = 1, linetype = "dashed", color = "black") +
#     scale_x_continuous(expand = c(0,0), limits = c(0, 1), breaks = seq(0, 1, .2), labels = function(x) x * 100) +
#     scale_y_continuous(expand = c(0,0), limits = c(0, 1), breaks = seq(0, 1, .2), labels = function(x) x * 100) +
#     labs(x = "1-Specificity", y = "Sensitivity") +
#     theme_bw2() +
#     theme(legend.position = "right", legend.key.height = unit(.275, "cm"), legend.title = element_blank())
...
save_plot(df_hot_auc_pl, "results/hotspot.pdf", w = 12, h = 8)
```
